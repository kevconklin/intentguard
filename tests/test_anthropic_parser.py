...
from engine.schema import DecideRequest, Decision, Mode
...
async def test_parse_validates_against_allowlist():
    ...
    assert [a.tool for a in intent.allowed_actions] == [
        "email.send"
    ]  # file.delete dropped
...
def test_parser_satisfies_intent_parser_protocol():
    assert isinstance(
        AnthropicIntentParser(registry=REG, extractor=_noop), IntentParser
    )
...
@pytest.mark.skipif(
    not (
        ...
        and os.environ.get("INTENTGUARD_TEST_ANTHROPIC")
    ),
