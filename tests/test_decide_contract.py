...
def _req(tool: str, args: dict, **kw) -> DecideRequest:
    return DecideRequest(session_id=SESSION, subject=SUBJECT, tool=tool, arguments=args, **kw)
...
async def test_deny_not_in_intent(store, enforce_config, audit, seeded):
    await seeded()
    resp = await decide(_req("email.send", {"to": "attacker@evil.com"}), store, enforce_config, audit)
    assert resp.decision == Decision.deny.value
    assert resp.reason == Reason.not_in_intent.value


async def test_deny_same_tool_different_resource_is_distinct_grant(store, enforce_config, audit, seeded):
    """email.send to bob is granted; email.send to carol is a different grant."""
    await seeded()
    resp = await decide(_req("email.send", {"to": "carol@example.com"}), store, enforce_config, audit)
    assert resp.decision == Decision.deny.value
...
async def test_escalate_when_configured(store, enforce_config, audit, seeded):
    await seeded()
    cfg = dataclasses.replace(enforce_config, escalatable_tools=frozenset({"email.send"}))
    resp = await decide(_req("email.send", {"to": "attacker@evil.com"}), store, cfg, audit)
    assert resp.decision == Decision.escalate.value
    assert resp.reason == Reason.escalated_for_review.value
    assert resp.escalation_prompt is not None and "attacker@evil.com" in resp.escalation_prompt
...
async def test_fail_closed_on_store_error(enforce_config, audit, seeded):
    ...
    resp = await decide(_req("email.send", {"to": "bob@example.com"}), FailingStore(), enforce_config, audit)
    assert resp.decision == Decision.deny.value
...
async def test_fail_closed_on_timeout(audit):
    ...
    resp = await decide(_req("email.send", {"to": "bob@example.com"}), SlowStore(delay=2.0), cfg, audit)
    assert resp.decision == Decision.deny.value
    assert resp.reason == Reason.pdp_error_failclosed.value
    assert audit.entries()[-1].error == "pdp_timeout"


async def test_observe_mode_allows_but_logs_would_have(store, observe_config, audit, seeded):
    await seeded()
    resp = await decide(_req("email.send", {"to": "attacker@evil.com"}), store, observe_config, audit)
    assert resp.decision == Decision.allow.value
