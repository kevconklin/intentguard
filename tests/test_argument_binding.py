...
def test_compound_binds_to_joined_parts_in_order():
    b = bind_resource(
        "http.request", {"host": "API.example.com", "path": "/Data"}, registry=REG
    )
...
def test_compound_distinguishes_grants():
    a = grant_object(
        ...
        "http.request",
        bind_resource(
            "http.request", {"host": "h", "path": "/a"}, registry=REG
        ).resource,
    )
    b = grant_object(
        SESSION,
        "http.request",
        bind_resource(
            "http.request", {"host": "h", "path": "/b"}, registry=REG
        ).resource,
    )
    assert a != b
...
async def test_compound_resource_roundtrip(store, writer, audit):
    ...
    ok = await decide(
        _req("http.request", {"host": "api.example.com", "path": "/data"}),
        store,
        cfg,
        audit,
    )
    ...
    wrong = await decide(
        _req("http.request", {"host": "api.example.com", "path": "/other"}),
        store,
        cfg,
        audit,
    )
